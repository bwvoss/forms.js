describe 'FormsJs.Form.Validator.Custom', ->

  it 'takes a custom regular expression and returns false when value does not match', ->
    customRegEx = /[^0-9]+/
    value = '12345'
    customValidator = new FormsJs.Form.Validator.Custom

    expect(customValidator.isValid(value, undefined, customRegEx)).toBeFalsy()

  it 'takes a custom regular expression and returns true when the value does match', ->
    customRegEx = /(?:\d{3}|\(\d{3}\))([-\/\.])\d{3}\1\d{4}/
    value = '123-456-7890'
    customValidator = new FormsJs.Form.Validator.Custom

    expect(customValidator.isValid(value, undefined, customRegEx)).toBeTruthy()

  it 'takes a custom regular expression and returns true when the value is blank and not required', ->
    customRegEx = /\W+/
    value = ''
    customValidator = new FormsJs.Form.Validator.Custom

    expect(customValidator.isValid(value, undefined, customRegEx)).toBeTruthy()

